...
def train( projection_head, model, train_loader, test_loader, unlabelled_train_loader, merge_train_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            if epoch%prototype_extraction_interval==0:
                uq_index, all_preds, cluster_protos_list, preds_ind_list,  metrics= \
                    extract_labeled_protos(model, merge_train_loader, epoch=epoch, args=args)
        ...
        # 判断最佳 checkpoint 的逻辑是，是否在测试集上的 Old 类达到最佳性能
        # 最终报告 Train ACC Unlabelled_v2 的准确率
        if old_acc_test > best_test_acc_lab:

            print(f'Best ACC on new Classes on disjoint test set: {new_acc_test:.4f}...')
            args.logger.info(f'Best ACC on new Classes on disjoint test set: {new_acc_test:.4f}...')
        ...
        args.logger.info('Best Train Epochs:  {}'.format(best_epoch))
    ...
    args.logger.info('Best Train Accuracies: All {:.4f} | Old {:.4f} | New {:.4f}'.format(best_stats[0], best_stats[1], best_stats[2]))
    args.logger.info('Best Train Epochs:  {} '.format(best_epoch))
...
def extract_labeled_protos(model, train_loader, epoch, args):
    model.eval()
    ...
    all_acc, old_acc, new_acc = log_accs_from_preds(y_true=u_targets.cpu().numpy(), y_pred=preds, mask=mask_cls,
                                                    ...
                                                    save_name='SS-K-Means Train ACC Unlabelled', print_output=True, writer=args.writer, args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='cub', help='options: cifar10, cifar100, scars, aircraft, herbarium_19, imagenet_100, xh, gt')
